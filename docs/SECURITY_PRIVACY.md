...
**方針**：結果と責任は常に説明可能にする。一方で、競争優位となる内部アルゴリズムは適切に保護する。

#### 必ず開示するもの（Non-negotiable Transparency）

以下は**必ず公開**する。ユーザー/企業/規制当局に対して常に説明可能な状態を維持する。

1. **Execution Ledger イベント**
   - `/approve` `/confirm` ごとの実行記録
   - 誰が/何を/いつ/どこで/どの経路で実行したか
   - 成功/失敗の状態と理由

2. **PoEx（実行レシート）の要素**
   - `device_sig`：端末署名（将来実装）
   - `server_sig`：サーバ署名
   - `merkle_root_week`：週次Merkle Root

3. **AXI / Security KPI**
   - 集計値（通話成功率/誤実行率/取消成功率等）
   - 定義と計測式
   - 週次更新の履歴
...
以下は**公開しない**。競争優位（Moat）として保護する。

1. **モデル構成・プロンプト**
   - LLMのプロンプト設計
   - Providerルーティングアルゴリズム
   - 内部実装の詳細

2. **vMB / FEA の内部計算**
   - 計算テーブルや重み
   - 公開するのは「値」と「保守的推定である」という方針のみ

3. **OrgごとのLoRA / Org Memory**
   - 各Orgの資産として扱う
   - 必要に応じてエクスポートAPIを提供

#### 目的

- **ユーザー/企業/規制当局**：結果と責任は常に説明できる
- **競合**：内部アルゴリズムや各社の暗黙知は適切に保護
- **バランス**：透明性と競争優位の両立
