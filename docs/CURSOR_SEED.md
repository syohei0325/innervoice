# =========================================
# Cursor Pack – Yohaku (7秒→2提案→1確定 / Confirm once / .ics / B2C Phone‑Hater Wedge / Doraemonモード / Pluggable Memory)
# =========================================
# 目的：Cursorが迷わずMVP→MVP+→先読みProactiveまで実装できるよう、文脈・ルール・受入基準・観測/KPI・安全規範を一式で提供
# 前提：0–6mは「B2C / PLG（電話が嫌いな個人向け）」モード。病院や事業者への営業/提携は行わないが、
#       将来（6m以降）のAction Cloud / Provider連携のための仕様は残す。
...
### コア体験（MVP：B2C Phone‑Hater Wedge「通話→予定化」）
- 入力：**7秒**（音声/テキスト）  
...
**結論**：  
0–6m は **B2C「電話嫌い」向けYohaku**として実運用データ（AXI/台帳）を積み、  
その後、**ConfirmOS / Execution Ledger / Call Provider Spec** を**Action Cloud**として中立API化する。  

**3段階の道筋（B2C → B2B2C）**
1) **Phase 1 – Yohaku B2C（0–9m）**：  
   - ターゲット：電話が嫌いな個人ユーザー  
   - ユースケース：病院予約 / 再配達 / サロン / 行政問い合わせ など  
   - 目的：7秒→2→1 UX と Call Rules（Ethics/Budget/Blacklist）の検証、AXIの週次公開。  
2) **Phase 1.5 – Action Cloud Private β（6–12mの一部）**：  
   - B2Cで得た実行データをもとに、**友好的なエージェント/LLMクライアントチーム 3–5社**へ  
     `/v1/plan → /v1/approve → /v1/confirm` を**SLAなしのPrivate β**として提供。  
   - 病院/事業者向けの Design Partner は、このフェーズ以降に検討（Yohaku経由のトラフィックを見せながら）。
3) **Phase 2 – Action Cloud β（12–24m）**：  
   - `/v1/plan → /v1/approve → /v1/confirm` を**SLA 99.5%**で外部に提供（中立・プロバイダ差し替え可）。  
   - B2B2Cとして予約SaaS / コールセンターBPO / 病院vertical などと提携。  
4) **Phase 3 – Action Cloud GA（24–36m）**：  
   - **Enterprise SLA 99.9%**（リージョン固定、監査証跡/EU DPA）  
   - **ConfirmOS v1**公開仕様＋**AXIリーダーボード**
...
- Doraemonモード（Proactive OS / Nudge / Relationship Graph / Partnerモード / Taste）は**0–6mでは実装しない**  
  （ログとデータモデルだけ用意。実行パスは封印）。
- Pluggable Memory / 外部Doc連携（Drive/Notion/メール等）は**PoCレベル**に留め、本番導入は**Action Cloud β以降**。
- OS Deep Integration / Browser Extension（Confirm Bar α）は、  
  **B2C Wedge（電話代行テンプレ）のAXIがGo基準を超えるまで後ろ倒し**。
- Public API / MCPは**友好的なエージェントチーム（3–5）向け Private β**のみに限定（一般公開はAction Cloud β以降）。
- 新verticalや新テンプレは、**B2C Wedge（電話代行テンプレ）のAXIが基準値を超えるまでは増やさない**  
  （やるとしても既存ユースケースのバリエーションに留める）。
...
1. **「電話が嫌いな個人」向けのB2Cプロダクトとして、毎週「確定したい用事」が十分に発生するか**  
   - 病院予約 / 再配達 / サロン / 行政など、電話が必要な用事だけでD30ユーザーが日あたり確定≥3を達成できるか。
...
**方針**：  
- これら4つが否定された場合は、**対象ユースケースの組み合わせ（病院/再配達/サロン/行政）やチャネル（電話以外）を早期に見直す。**  
- B2CでのPMFが難しい場合も、ConfirmOS/Execution Ledger/Call Provider Specの資産を活かした**Action Cloud単体SaaS**へのピボットを常にオプションとして保持する。
...
## 新規：Memory
- `POST /api/memory.put`  — 例:

    { "kind":"preference","key":"coffee.sugar","value":0,"ttl_days":365 }

- `POST /api/memory.forget`— 例:

    { "key":"coffee.sugar" }

- `POST /api/memory.query` — 例:
...
0) **B2C / PLG ループ（電話嫌いユーザー起点）**：  
   ...
   - Founder Story / Buildログ（YouTube/X）で「電話筋肉ゼロ化への挑戦」を継続共有。

0.5) **Design Partner / Aggregator プログラム（6m以降）**：  
   - B2Cで一定のトラフィックとAXI実績が出た後、一次診療クリニックverticalや予約SaaS/BPOと組み、  
     **Action Cloud連携**（`/confirm`）を標準機能として組み込んでもらう。  
   - 0–6mでは準備＆対話レベルに留め、営業/導入は急がない。
...
Now–6m：B2C電話代行（Cal/Message/Reminders/通話テンプレ & Car KPI本番）  
6–12m：Taste v1 / Partner β / MCP: places,reservations,parking,ride（署名/審査/スコープ）＋ Action Cloud Private β  
...
# ─────────────────────────
## 0–6m（B2C / PLG フェーズ）
- **Yohaku Wedge**：B2C電話代行（「電話が嫌いな個人」向け）で  
  病院予約 / 再配達 / サロン / 行政問い合わせ 等のテンプレを本番運用。  
  事業者側への営業/提携は行わず、**Call Rules（Ethics/Budget/Blacklist）**を実装して迷惑電話化を防ぐ。
...
- **Design Partner / Aggregatorはリサーチのみ**：  
  病院/予約SaaS/BPOなどと、非公式なヒアリングやPoCレベルの対話は行うが、0–6mでは正式な営業/導入は急がない。
- **非コア機能の凍結**：Doraemonモード / Proactive OS / Taste / Relationship Graph / Browser Extension / OS Deep Integrations / Pluggable Memory本番運用は0–6mでは**実装しない**（データモデルとロギングだけ先に用意）。リソースはB2C Wedgeの品質とAXI改善・Call Rulesに集中。

## 6–12m
- **Action Cloud Private β（招待制）**：`/v1/plan → /v1/approve → /v1/confirm` を **SLAなし**で友好的なエージェント/LLMクライアント3–5チームへ提供（中立）。  
  B2Cで得たAXI/台帳を元にフィードバック取得。
- **初期Design Partner候補とのPoC**：  
  一次診療クリニックverticalや予約SaaS/BPOとの小規模PoCを実施（Yohaku経由トラフィックを見せながら）。  
...
## 移行ロードマップ（Yohaku B2C → Action Cloud）
1) **Yohaku実運用（B2C）**：  
   B2C電話代行プロダクトとして、電話嫌いな個人向けに複数ユースケース（病院/再配達/サロン/行政）を本番運用。  
   **AXIを週次公開**、台帳席課金の設計。  
2) **Private β（エージェント向け・超少数）**：  
   B2CでのUX/指標が安定した後、友好的なエージェント/LLMクライアント3–5チームに  
   `/v1/plan → /v1/approve → /v1/confirm` を提供（SLAなし。フィードバックと実行データ取得が目的）。  
3) **β（招待制）**：  
   デザインパートナー20社／**SLA 99.5%**／**プロバイダ中立**（Twilio/Telnyx他）  
4) **GA**：**ConfirmOS v1**公開／**AXIリーダーボード**／EU DPA対応／監査証跡標準
...
> 0–6mは「B2C電話代行」テンプレが本番運用の主戦場。  
> 病院/再配達/サロン/行政など、電話が必要だがユーザーがかけたくない領域を優先する。

## 病院予約（一次診療）
Plan.summary: 「◯◯クリニックに明日10:30で予約／家族へ共有／9:45リマインド」
Plan.actions:
- { action: "call.place", to: "+81XXXXXXXX", script: "◯◯さんご本人のご依頼でお電話しています。受診予約を取りたいです。明日10時半は空いていますか？", locale: "ja-JP", record: false }
...
## 再配達/配送業者対応
Plan.summary: 「再配達を明日19:00以降の枠で確定／カレンダー＋通知」
Plan.actions:
- { action: "call.place", to: "+81YYYYYYYY", script: "◯◯さんご本人のご依頼でお電話しています。不在票の再配達をお願いしたいのですが、明日19時以降で可能な枠はありますか？", locale: "ja-JP", record: false }
- { action: "calendar.create", title: "再配達", start: "2025-10-19T19:00:00+09:00", duration_min: 60 }
- { action: "reminder.create", time: "2025-10-19T18:45:00+09:00", note: "在宅待機" }

## 美容室/サロン予約
Plan.summary: 「◯◯サロンでカット予約／19:00／カレンダー＋相手に共有メッセ」
Plan.actions:
- { action: "call.place", to: "+81ZZZZZZZZ", script: "◯◯さんご本人のご依頼でお電話しています。明日19時ごろにカットの予約は可能でしょうか？", locale: "ja-JP", record: false }
- { action: "calendar.create", title: "Haircut", start: "2025-10-19T19:00:00+09:00", duration_min: 60 }
- { action: "message.send", to: "本人", text: "19時に◯◯サロンで予約取れたよ" }
- { action: "reminder.create", time: "2025-10-19T18:30:00+09:00", note: "出発準備" }

## 行政・役所問い合わせ
Plan.summary: 「区役所に住民票の取得方法を確認／要点要約＋TODOリスト」
Plan.actions:
- { action: "call.place", to: "+81AAAAAAA", script: "◯◯さんご本人のご依頼でお電話しています。住民票の取得方法と必要な持ち物を教えていただけますか？", locale: "ja-JP", record: false }
- { action: "reminder.create", time: "2025-10-19T21:00:00+09:00", note: "持ち物を準備する" }
- { action: "docs.create", type: "checklist", note: "住民票取得に必要なもの" }  ※docs.createは将来拡張
...
## 12–18か月の実行計画
### Phase A（0–90日）… **8/10へ**
- AXI週次公開、**B2C電話代行Wedge**で**通話成功≥90%/誤実行<0.5%**  
...
# ─────────────────────────
# CHANGELOG
- **2025-12-06**: 戦略を「B2C / PLG（電話嫌いユーザー向け）集中＋Action Cloud Private β（6m以降）＋病院vertical/Aggregatorは将来オプション」に更新。README/PRD_MVP/VISION/ROADMAP_36M/DISTRIBUTION_PLAYBOOK/CALL_TEMPLATES/ACTION_CLOUDを修正し、0–6mはB2C電話代行Wedgeにフォーカスする方針を明記。Call Rules（Ethics/Budget/Blacklist）を追加。
...
- **2025-11-06**: READMEに「Yohaku → Action Cloud（最大EVへの道筋）」を追記。`docs/ACTION_CLOUD.md` を新設。`docs/ROADMAP_36M.md` にAction Cloudのβ/GAマイルストーンを明記。DISTRIBUTIONにEarly Accessを追加。
